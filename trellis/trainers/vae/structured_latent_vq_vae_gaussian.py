...
class SLatVqVaeGaussianTrainer(BasicTrainer):
    """
    Trainer for structured latent VQ-VAE.
    ...
    def training_losses(
        ...
        **kwargs
        ...
        z = self.training_models['encoder'](feats, sample_posterior=False, return_raw=False)
        loss, quantized, perplexity, _ = self.training_models['vq'](z.feats)
        z = z.replace(quantized)
        ...
        terms["vq_commitment"] = loss
        terms["loss"] = terms["loss"] + terms["vq_commitment"]
        terms["perplexity"] = perplexity
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            z = self.models['encoder'](args['feats'], sample_posterior=False, return_raw=False)
            
            ref_reps.extend(self.models['decoder'](z))
            
            _, quantized, _, _ = self.models['vq'](z.feats)
            z = z.replace(quantized)
        ...
        ref_render_results = self._render_batch(ref_reps, exts, ints)

        ret_dict.update({f'rec_image': {'value': render_results['color'], 'type': 'image'}})
        ret_dict.update({f'ref_image': {'value': ref_render_results['color'], 'type': 'image'}})
